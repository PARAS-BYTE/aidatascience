...
from app.db.models import Dataset, TaskType
...
def suggest_target(dataset_id: str, db: Session = Depends(get_db)):
...
def detect_task(dataset_id: str, target: str, db: Session = Depends(get_db)):
